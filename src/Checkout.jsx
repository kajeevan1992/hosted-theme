...
import { AlertTriangle, Check, Clock, CreditCard, FileQuestion, FileText, Image as ImageIcon, MapPin, PackageCheck, ShieldCheck, Truck, Upload } from "lucide-react";
...
function itemProductKey(item = {}) { return item.productId || item.slug || item.productSlug || item.product_id || item.id?.split?.("-")?.[0] || item.name; }
...
function lineVatProfile(item = {}) {
  ...
  const explicit = [item.vatRate, item.taxRate, pricing.vatRate, selected.vatRate, item.config?.vatRate, item.selections?.vatRate].find((v) => Number.isFinite(Number(v)));
  ...
  if (["leaflet", "flyer", "booklet", "brochure"].some((term) => name.includes(term))) return { rate: 0, label: "0% VAT", reason: "Zero-rated print" };
...
function StepPill({ label, index, current }) {
  const done = current > index; const active = current === index;
...
function buildCollectionOption(location = {}) {
  ...
  return {
    ...
    collectionHours: location.collectionHours || location.openingHours || [],
...
export default function Checkout({ cart, navigate }) {
  ...
  useEffect(() => {
    ...
    async function reprice() {
      ...
      for (const item of items) {
        const productId = itemProductKey(item); const quantity = itemQty(item); const selections = itemSelections(item);
        try {
      ...
      if (!cancelled) { setResolvedLines(next); setResolverStatus(next.some((line) => line.resolverError) ? "fallback" : "resolved"); }
    }
    reprice();
  ...
  const itemGrossSubtotal = checkoutItems.reduce((sum, item) => sum + Number(item.price || 0) * itemQty(item), 0);
  ...
  useEffect(() => {
    ...
    async function loadDelivery() {
      ...
      try {
        const payload = await resolveDeliveryOptions({ postcode: effectivePostcode, subtotalMinor: minorFromMoney(itemGrossSubtotal) });
        if (cancelled) return;
        const options = payload?.data?.options || payload?.options || [];
        setDeliveryOptions(Array.isArray(options) ? options.map(buildDeliveryOption) : []);
    ...
    loadDelivery();
    return () => { cancelled = true; };
  }, [effectivePostcode, itemGrossSubtotal]);
  ...
  useEffect(() => {
    ...
    async function loadLocations() {
      ...
      try {
        ...
        if (cancelled) return;
        setCollectionLocations(locations.length ? locations : fallbackLocations());
        setLocationStatus(locations.length ? "resolved" : "fallback");
      } catch {
        if (cancelled) return;
        setCollectionLocations(fallbackLocations());
  ...
  const fallbackDeliveryOptions = [
    { id: "standard-fallback", publicLabel: "Standard delivery (3–5 working days)", checkoutDescription: "Best-value default delivery option", priceMinor: 0, fulfilmentMode: "delivery", vatRate: 20 },
    { id: "express-fallback", publicLabel: "Express delivery (24–48 hours)", checkoutDescription: "Faster production and dispatch", priceMinor: 995, fulfilmentMode: "delivery", vatRate: 20 },
    { id: "priority-fallback", publicLabel: "Priority same-day review", checkoutDescription: "Urgent review and production queue", priceMinor: 1495, fulfilmentMode: "local-courier", requiresManualApproval: true, vatRate: 20 },
  ];
  const availableDeliveryOptions = (deliveryOptions.length ? deliveryOptions : fallbackDeliveryOptions).map(buildDeliveryOption);
  const collectionOptions = collectionLocations.filter((loc) => loc.type === "main-store" || loc.type === "owned-branch" || loc.type === "partner-collection-point").map(buildCollectionOption);
  const fulfilmentOptions = [...availableDeliveryOptions, ...collectionOptions];
  const selectedFulfilment = fulfilmentOptions.find((option) => String(optionId(option)) === String(form.fulfilmentChoice || form.delivery)) || fulfilmentOptions[0];
  const fulfilmentFee = deliveryPrice(selectedFulfilment);
  const taxSummary = useMemo(() => buildTaxSummary(checkoutItems, fulfilmentFee, selectedFulfilment), [JSON.stringify(checkoutItems.map((item) => ({ id: item.id, price: item.price, q: itemQty(item), vatRate: item.vatRate, key: itemProductKey(item) }))), fulfilmentFee, selectedFulfilment?.id, selectedFulfilment?.vatRate]);
  const preflightBlocked = artworkState.mode === "now" && artworkState.preflight?.preflight?.status === "blocked";
  const quoteRequired = checkoutItems.some(hasQuoteFlag) || checkoutItems.some((item) => item.quoteRequired || item.checkoutBlocked) || selectedFulfilment?.requiresManualApproval || Boolean(artworkState.preflight?.preflight?.requiresManualReview);
  const checkoutBlocked = checkoutItems.some((item) => item.checkoutBlocked || item.resolverSnapshot?.checkout?.blocked) || preflightBlocked;

  useEffect(() => {
    if (!form.fulfilmentChoice && fulfilmentOptions[0]) {
      const id = optionId(fulfilmentOptions[0]);
      setForm((prev) => ({ ...prev, fulfilmentChoice: id, delivery: id }));
    }
  }, [fulfilmentOptions.length]);

  async function runArtworkPreflight(file) {
    if (!file) return;
    setArtworkState((prev) => ({ ...prev, preflightStatus: "checking", preflight: null, error: "" }));
    const firstItem = checkoutItems[0] || items[0] || {};
    try {
      const result = await resolveArtworkPreflight({ productId: itemProductKey(firstItem), files: [fileMeta(file)], selections: itemSelections(firstItem), artworkMode: artworkState.mode });
      setArtworkState((prev) => ({ ...prev, preflight: result, preflightStatus: result?.preflight?.status || "passed", error: result?.preflight?.errors?.join(" ") || "" }));
    } catch (error) {
      setArtworkState((prev) => ({ ...prev, preflightStatus: "fallback", preflight: null, error: error?.message || "Artwork preflight is unavailable. You can still upload for manual review." }));
    }
  }

  const canContinue = useMemo(() => {
    if (step === 1) return !!form.firstName && !!form.lastName && !!form.email;
    if (step === 3) { const billingOk = !!form.billingAddress1 && !!form.billingCity && !!form.billingPostcode; const deliveryOk = form.sameAsBilling || (!!form.deliveryAddress1 && !!form.deliveryCity && !!form.deliveryPostcode); return billingOk && deliveryOk; }
    if (step === 4) return !!selectedFulfilment;
    if (step === 5) return artworkState.mode === "later" || ((!!artworkState.file || !!artworkState.uploaded) && !preflightBlocked);
    if (step === 7) return quoteRequired ? form.agree && !preflightBlocked : (!!form.paymentMethod && form.agree && !checkoutBlocked);
    return true;
  }, [step, form, artworkState, selectedFulfilment, quoteRequired, checkoutBlocked, preflightBlocked]);

  async function handleArtworkUpload() {
    if (!artworkState.file || preflightBlocked) return;
    const firstItem = checkoutItems[0] || items[0] || {};
    try {
      const res = await uploadArtwork(artworkState.file, { mode: artworkState.mode, preflight: artworkState.preflight, productId: itemProductKey(firstItem), slug: itemProductKey(firstItem) });
      setArtworkState((prev) => ({ ...prev, uploaded: res, error: "" }));
    } catch (error) {
      setArtworkState((prev) => ({ ...prev, error: error?.message || "Artwork upload failed" }));
    }
  }

  function customerName() { return `${text(form.firstName)} ${text(form.lastName)}`.trim(); }
  function buildPayload() {
    const fulfilment = selectedFulfilment || {};
    const isCollection = String(fulfilment.fulfilmentMode || fulfilment.type || "").includes("collection");
    const customer = { firstName: form.firstName, lastName: form.lastName, name: customerName(), email: form.email, phone: form.phone, company: form.companyName, vatNumber: form.vatNumber };
    ...
    return {
      source: "hosted-theme-checkout-build-60",
      customer,
      ...
      deliveryAddress,
      fulfilmentMode: fulfilment.fulfilmentMode || fulfilment.type || "delivery",
      fulfilmentChoice: optionId(fulfilment),
      fulfilmentSelection: {
        id: optionId(fulfilment),
        label: deliveryLabel(fulfilment),
        description: deliveryDescription(fulfilment),
        mode: fulfilment.fulfilmentMode || fulfilment.type || "delivery",
        isCollection,
        priceMinor: minorFromMoney(fulfilmentFee),
        vatRate: Number(fulfilment.vatRate ?? 20),
        ...
        requiresManualApproval: Boolean(fulfilment.requiresManualApproval),
      },
      delivery: { id: optionId(fulfilment), label: deliveryLabel(fulfilment), price: fulfilmentFee, priceMinor: minorFromMoney(fulfilmentFee), vatRate: Number(fulfilment.vatRate ?? 20), fulfilmentMode: fulfilment.fulfilmentMode || fulfilment.type || "delivery", locationSlug: fulfilment.locationSlug || "" },
  ...
  async function handleSubmit() {
    setSubmitting(true); setQuoteSubmitState(null);
    const payload = buildPayload();
    try {
      if (quoteRequired || checkoutBlocked) {
        const res = await createQuoteRequest({ productId: payload.items[0]?.productId, customer: payload.customer, selections: payload.items, artwork: payload.artwork_reference, artworkPreflight: payload.artwork_preflight, notes: payload.notes, checkout: payload });
        setQuoteSubmitState(res);
        setSubmitState({ success: true, message: "Quote request captured. We will review the specification, collection/delivery and artwork route before confirming price/payment." });
      } else {
        const res = await createOrder(payload);
        setSubmitState(res?.success === false ? res : { success: true, message: "Order submitted. If you selected pay now, secure payment will open next.", response: res });
      }
      setStep(8);
    } catch (error) {
      setSubmitState({ success: false, message: error?.message || "Checkout submission failed. Please try again." }); setStep(8);
    } finally { setSubmitting(false); }
  }

  const inputClass = "h-11 rounded-xl border px-3 text-[13px]";

  return <section className="py-6"><div className="mx-auto max-w-[1000px] px-4 sm:px-6 lg:px-8"><div className="mb-5 flex items-end justify-between gap-4"><div><div className="text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: "rgb(24, 167, 208)" }}>Checkout</div><h1 className="mt-2 text-[28px] font-black tracking-[-0.04em]" style={{ color: "#121517" }}>Secure print order checkout</h1><p className="mt-2 max-w-[760px] text-[12px] leading-6" style={{ color: "#667179" }}>Choose delivery or collection, upload artwork, review VAT and submit payment or quote approval.</p></div><button onClick={() => navigate("/cart")} className="rounded-full border px-4 py-2 text-[12px] font-bold" style={{ borderColor: "#E2E6E8", color: "#121517", backgroundColor: "white" }}>Back to cart</button></div><div className="mb-5 flex flex-wrap gap-2">{STEPS.map((label, i) => <StepPill key={label} label={label} index={i + 1} current={step} />)}</div><div className="mb-5 grid gap-3 md:grid-cols-4"><ResolverNotice type={resolverStatus === "resolved" ? "ok" : resolverStatus === "fallback" ? "warn" : "info"}>Cart price: {resolverStatus}</ResolverNotice><ResolverNotice type={deliveryStatus === "resolved" ? "ok" : deliveryStatus === "fallback" ? "warn" : "info"}>Delivery: {deliveryStatus}</ResolverNotice><ResolverNotice type={locationStatus === "resolved" ? "ok" : locationStatus === "fallback" ? "warn" : "info"}>Collection: {locationStatus}</ResolverNotice><ResolverNotice type={quoteRequired ? "warn" : "ok"}>{quoteRequired ? "Quote/manual review" : "Normal checkout"}</ResolverNotice></div><div className="grid gap-6 lg:grid-cols-[1fr_360px]"><div className="rounded-[22px] border bg-white p-5 shadow-[0_12px_28px_rgba(0,0,0,0.03)]" style={{ borderColor: "#E2E6E8" }}>
    {step === 1 && <div className="grid gap-4 sm:grid-cols-2"><Field label="First name"><input value={form.firstName} onChange={(e) => setField("firstName", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field><Field label="Last name"><input value={form.lastName} onChange={(e) => setField("lastName", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field><Field label="Email"><input value={form.email} onChange={(e) => setField("email", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field><Field label="Phone" hint="Optional"><input value={form.phone} onChange={(e) => setField("phone", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field></div>}
    {step === 2 && <div className="grid gap-4 sm:grid-cols-2"><Field label="Company name" hint="Optional"><input value={form.companyName} onChange={(e) => setField("companyName", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field><Field label="VAT number" hint="Optional"><input value={form.vatNumber} onChange={(e) => setField("vatNumber", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field><div className="sm:col-span-2 rounded-[16px] border bg-[#F8FBFC] p-4 text-[12px]" style={{ borderColor: "#E2E6E8", color: "#667179" }}>Add company details if this is a business order or if you need invoicing information on the order.</div></div>}
    {step === 3 && <div className="grid gap-5"><SectionCard title="Billing address" eyebrow="Billing"><div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Field label="Address line 1"><input value={form.billingAddress1} onChange={(e) => setField("billingAddress1", e.target.value)} className={`${inputClass} w-full`} style={{ borderColor: "#E2E6E8" }} /></Field></div><Field label="City"><input value={form.billingCity} onChange={(e) => setField("billingCity", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field><Field label="Postcode"><input value={form.billingPostcode} onChange={(e) => setField("billingPostcode", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field></div></SectionCard><label className="flex items-center gap-3 rounded-[14px] border bg-[#FBFCFD] px-4 py-3 text-[12px]" style={{ borderColor: "#E2E6E8", color: "#667179" }}><input type="checkbox" checked={form.sameAsBilling} onChange={(e) => setField("sameAsBilling", e.target.checked)} />Delivery address is the same as billing</label>{!form.sameAsBilling && <SectionCard title="Delivery address" eyebrow="Delivery"><div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Field label="Address line 1"><input value={form.deliveryAddress1} onChange={(e) => setField("deliveryAddress1", e.target.value)} className={`${inputClass} w-full`} style={{ borderColor: "#E2E6E8" }} /></Field></div><Field label="City"><input value={form.deliveryCity} onChange={(e) => setField("deliveryCity", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field><Field label="Postcode"><input value={form.deliveryPostcode} onChange={(e) => setField("deliveryPostcode", e.target.value)} className={inputClass} style={{ borderColor: "#E2E6E8" }} /></Field></div></SectionCard>}</div>}
    ...
    {step === 7 && <div className="grid gap-4"><SectionCard title={quoteRequired ? "Submit quote request" : "Choose payment"} eyebrow="Payment"><div className="grid gap-3"><label className="rounded-[14px] border bg-white p-4" style={{ borderColor: form.paymentMethod === "Pay now" ? "rgb(24, 167, 208)" : "#E2E6E8" }}><input type="radio" checked={form.paymentMethod === "Pay now"} onChange={() => setField("paymentMethod", "Pay now")} disabled={quoteRequired} /> <CreditCard className="mx-2 inline h-4 w-4" /> Pay now</label><label className="rounded-[14px] border bg-white p-4" style={{ borderColor: form.paymentMethod === "Invoice" ? "rgb(24, 167, 208)" : "#E2E6E8" }}><input type="radio" checked={form.paymentMethod === "Invoice"} onChange={() => setField("paymentMethod", "Invoice")} disabled={quoteRequired} /> Request invoice / account payment</label><textarea value={form.notes} onChange={(e) => setField("notes", e.target.value)} placeholder="Order notes" className="min-h-[90px] rounded-xl border p-3 text-[13px]" style={{ borderColor: "#E2E6E8" }} /><label className="flex gap-3 text-[12px]" style={{ color: "#667179" }}><input type="checkbox" checked={form.agree} onChange={(e) => setField("agree", e.target.checked)} />I confirm the order details, VAT summary, collection/delivery choice and artwork choice are correct.</label></div></SectionCard></div>}
  ...
  </div><aside className="h-fit rounded-[22px] border bg-white p-5 shadow-[0_12px_28px_rgba(0,0,0,0.03)]" style={{ borderColor: "#E2E6E8" }}><div className="mb-3 text-[15px] font-black" style={{ color: "#121517" }}>Order summary</div><div className="grid gap-3">{checkoutItems.map((item, index) => <div key={item.id || index} className="rounded-[14px] border p-3" style={{ borderColor: "#E2E6E8" }}><div className="text-[13px] font-bold" style={{ color: "#121517" }}>{item.name || item.productName || itemProductKey(item)}</div><div className="mt-1 text-[12px]" style={{ color: "#667179" }}>Qty {itemQty(item)} · {currency(Number(item.price || 0) * itemQty(item))}</div><div className="mt-1 text-[11px]" style={{ color: "#667179" }}>{lineVatProfile(item).label} · {lineVatProfile(item).reason}</div></div>))}</div><div className="mt-4 grid gap-2 border-t pt-4 text-[13px]" style={{ borderColor: "#E2E6E8", color: "#667179" }}><div className="flex justify-between"><span>Net</span><b>{currency(taxSummary.net)}</b></div><div className="flex justify-between"><span>VAT</span><b>{currency(taxSummary.vat)}</b></div><div className="flex justify-between"><span>Collection / delivery</span><b>{fulfilmentFee ? currency(fulfilmentFee) : "Free"}</b></div><div className="flex justify-between text-[16px]" style={{ color: "#121517" }}><span>Total</span><b>{currency(taxSummary.gross)}</b></div></div><div className="mt-4 rounded-[14px] border bg-[#F8FBFC] p-3 text-[12px] leading-6" style={{ borderColor: "#E2E6E8", color: "#667179" }}><div className="mb-1 flex items-center gap-2 font-bold" style={{ color: "#121517" }}><MapPin className="h-4 w-4" />Selected fulfilment</div>{selectedFulfilment ? <><div>{deliveryLabel(selectedFulfilment)}</div><div>{selectedFulfilment.cutoffTime ? `Cutoff ${selectedFulfilment.cutoffTime}` : "Cutoff confirmed after review"}</div></> : "Choose an option in step 4."}</div><div className="mt-4 grid gap-2">{taxSummary.breakdown.map((row) => <div key={row.rate} className="rounded-[12px] border bg-white px-3 py-2 text-[11px]" style={{ borderColor: "#E2E6E8", color: "#667179" }}><b style={{ color: "#121517" }}>{row.label}</b> · Net {currency(row.net)} · VAT {currency(row.vat)}</div>)}</div></aside></div></div></section>;
}
