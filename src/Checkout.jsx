...
import { AlertTriangle, Check, CreditCard, FileQuestion, FileText, Image as ImageIcon, ShieldCheck, Truck, Upload } from "lucide-react";
...
function readLocalCartItems() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem("holo-cart") || "[]");
...
function hasQuoteFlag(item = {}) {
  return Boolean(item.quoteRequired || item.checkoutBlocked || item.checkout?.quoteRequired || item.checkout?.blocked || item.pricing?.quoteRequired || item.resolverSnapshot?.checkout?.quoteRequired || item.resolverSnapshot?.checkout?.blocked);
}

function fileMeta(file) {
  if (!file) return null;
  return { name: file.name, type: file.type, size: file.size, sizeBytes: file.size };
}
...
function Field({ label, children, hint = "" }) {
  return <label className="grid gap-2"><div className="flex items-center justify-between gap-3"><span className="text-[12px] font-bold" style={{ color: "#121517" }}>{label}</span>{hint ? <span className="text-[11px]" style={{ color: "#667179" }}>{hint}</span> : null}</div>{children}</label>;
}
...
function ResolverNotice({ type = "info", children }) {
  ...
  return <div className="rounded-[14px] border px-4 py-3 text-[12px] leading-6" style={{ borderColor: isBad ? "#FCA5A5" : isWarn ? "#FACC15" : isOk ? "#86EFAC" : "#BAE6FD", backgroundColor: isBad ? "#FEF2F2" : isWarn ? "#FEFCE8" : isOk ? "#F0FDF4" : "#F0F9FF", color: isBad ? "#991B1B" : isWarn ? "#854D0E" : isOk ? "#166534" : "#075985" }}>{children}</div>;
}
...
export default function Checkout({ cart, navigate }) {
  ...
  const [form, setForm] = useState({
    ...
    delivery: "", paymentMethod: "Pay now", notes: "", agree: false,
  });
    ...
    async function reprice() {
      ...
      for (const item of items) {
        ...
        try {
          ...
          next.push({ ...item, qty: quantity, quantity, price: resolvedMinor != null ? moneyFromMinor(resolvedMinor) : Number(item.price || 0), resolverSnapshot: resolved, quoteRequired: Boolean(resolved?.checkout?.quoteRequired), checkoutBlocked: Boolean(resolved?.checkout?.blocked), resolverMessages: resolved?.checkout?.messages || resolved?.appliedRules?.messages || [] });
        } catch (error) {
  ...
  const quoteRequired = checkoutItems.some(hasQuoteFlag) || checkoutItems.some((item) => item.quoteRequired || item.checkoutBlocked) || selectedDelivery?.requiresManualApproval || Boolean(artworkState.preflight?.preflight?.requiresManualReview);
  ...
  const canContinue = useMemo(() => {
    ...
    return true;
  }, [step, form, artworkState, selectedDelivery, quoteRequired, checkoutBlocked, preflightBlocked]);

  async function handleArtworkUpload() {
    if (!artworkState.file || preflightBlocked) return;
    const res = await uploadArtwork(artworkState.file, { mode: artworkState.mode, preflight: artworkState.preflight });
  ...
  function buildPayload() {
    return {
      ...
      artwork_preflight: artworkState.preflight || null,
      items: checkoutItems.map((item) => ({ name: item.name, productId: itemProductKey(item), qty: itemQty(item), price: item.price, config: itemSelections(item), resolverSnapshot: item.resolverSnapshot || null, quoteRequired: Boolean(item.quoteRequired || item.resolverSnapshot?.checkout?.quoteRequired), checkoutBlocked: Boolean(item.checkoutBlocked || item.resolverSnapshot?.checkout?.blocked) })),
      resolver: { status: resolverStatus, deliveryStatus, artworkPreflightStatus: artworkState.preflightStatus, quoteRequired, checkoutBlocked },
