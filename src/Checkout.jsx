...
import { trackCheckoutEvent, trackEvent } from "./analytics/ga4";
...
export default function Checkout({ cart, navigate }) {
  ...
  useEffect(() => { trackCheckoutEvent('begin_checkout', { items: checkoutItems, grossTotalMinor: minorFromMoney(taxSummary.gross), currency: 'GBP' }, { item_count: checkoutItems.length }); }, []);
  ...
  async function handleSubmit() {
    ...
    try {
      if (quoteRequired || checkoutBlocked) {
        ...
        trackCheckoutEvent('generate_lead', { items: payload.items, grossTotalMinor: payload.totals?.grossTotalMinor, currency: 'GBP' }, { fulfilment_mode: payload.fulfilmentMode, quote_required: true });
        ...
        trackCheckoutEvent('purchase', { items: payload.items, grossTotalMinor: payload.totals?.grossTotalMinor, currency: 'GBP' }, { transaction_id: response?.order?.orderNumber || response?.orderNumber || payload.quoteReference, fulfilment_mode: payload.fulfilmentMode });
        setSubmitState({ success: true, message: "Order submitted. If you selected pay now, secure payment will open next.", response });
      }
      trackEvent('checkout_fulfilment_selected', { fulfilment_mode: payload.fulfilmentMode, fulfilment_choice: payload.fulfilmentChoice, shipping_method: payload.shippingMethod });
      setStep(8);
    } catch (error) {
      trackEvent('checkout_error', { message: error?.message || 'Checkout submission failed', fulfilment_mode: payload.fulfilmentMode });
  ...
  return <section className="py-6"><div className="mx-auto max-w-[1000px] px-4 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]"><div className="rounded-[22px] border bg-white p-5 shadow-[0_12px_28px_rgba(0,0,0,0.03)]" style={{ borderColor: "#E2E6E8" }}>
      ...
      {step === 7 && <div className="grid gap-4"><SectionCard title="Payment choice" eyebrow="Payment"><div className="grid gap-3"><label className="rounded-[14px] border bg-white p-4 text-[13px]" style={{ borderColor: form.paymentMethod === "Pay now" ? "rgb(24,167,208)" : "#E2E6E8" }}><input className="mr-2" type="radio" checked={form.paymentMethod === "Pay now"} onChange={() => setField("paymentMethod", "Pay now")} />Pay now by card</label><label className="rounded-[14px] border bg-white p-4 text-[13px]" style={{ borderColor: form.paymentMethod === "Quote request" ? "rgb(24,167,208)" : "#E2E6E8" }}><input className="mr-2" type="radio" checked={form.paymentMethod === "Quote request"} onChange={() => setField("paymentMethod", "Quote request")} />Request quote / payment link</label></div></SectionCard><label className="flex items-start gap-3 rounded-[14px] border bg-[#FBFCFD] p-4 text-[12px]" style={{ borderColor: "#E2E6E8", color: "#667179" }}><input type="checkbox" checked={form.agree} onChange={(event) => setField("agree", event.target.checked)} />I confirm the order details, VAT summary, artwork route and collection/delivery choice are correct.</label></div>}
      {step === 8 && <div className="grid gap-4"><ResolverNotice type={submitState?.success ? "ok" : "bad"}>{submitState?.message || "Checkout complete."}</ResolverNotice>{quoteSubmitState && <SectionCard title="Quote request reference" eyebrow="Saved"><pre className="overflow-auto rounded-xl bg-[#121517] p-3 text-[11px] text-white">{JSON.stringify(quoteSubmitState, null, 2)}</pre></SectionCard>}<button onClick={() => navigate("/")} className="rounded-full px-4 py-3 text-[13px] font-bold text-white" style={{ backgroundColor: "rgb(24, 167, 208)" }}>Back to home</button></div>}
    </div><aside className="rounded-[22px] border bg-white p-5 shadow-[0_12px_28px_rgba(0,0,0,0.03)]" style={{ borderColor: "#E2E6E8" }}><div className="text-[16px] font-black" style={{ color: "#121517" }}>Order summary</div><div className="mt-4 grid gap-3">{checkoutItems.map((item, index) => <div key={item.id || index} className="rounded-[14px] border bg-[#FBFCFD] p-3 text-[12px]" style={{ borderColor: "#E2E6E8" }}><div className="font-bold" style={{ color: "#121517" }}>{item.name || item.productName || itemProductKey(item)}</div><div style={{ color: "#667179" }}>Qty {itemQty(item)} · {currency(item.price || 0)} each</div><div className="mt-1 text-[11px]" style={{ color: "#667179" }}>{lineVatProfile(item).label} · {lineVatProfile(item).reason}</div></div>)}</div><div className="mt-4 grid gap-2 border-t pt-4 text-[13px]" style={{ borderColor: "#E2E6E8" }}><div className="flex justify-between"><span>Net</span><b>{currency(taxSummary.net)}</b></div><div className="flex justify-between"><span>VAT</span><b>{currency(taxSummary.vat)}</b></div><div className="flex justify-between"><span>Fulfilment</span><b>{currency(taxSummary.fulfilment)}</b></div><div className="flex justify-between text-[16px]"><span>Total</span><b>{currency(taxSummary.gross)}</b></div></div>{taxSummary.breakdown.length ? <div className="mt-4 rounded-[14px] border bg-[#FBFCFD] p-3 text-[11px] leading-5" style={{ borderColor: "#E2E6E8", color: "#667179" }}>{taxSummary.breakdown.map((row) => <div key={row.rate}>{row.label}: Net {currency(row.net)} · VAT {currency(row.vat)} · Gross {currency(row.gross)}</div>)}</div> : null}<div className="mt-4 flex gap-2"><button disabled={step === 1} onClick={() => setStep((s) => Math.max(1, s - 1))} className="flex-1 rounded-full border px-4 py-3 text-[12px] font-bold disabled:opacity-40" style={{ borderColor: "#E2E6E8" }}>Back</button>{step < 7 && <button disabled={!canContinue} onClick={() => setStep((s) => Math.min(7, s + 1))} className="flex-1 rounded-full px-4 py-3 text-[12px] font-bold text-white disabled:opacity-40" style={{ backgroundColor: "#121517" }}>Continue</button>}{step === 7 && <button disabled={!canContinue || submitting} onClick={handleSubmit} className="flex-1 rounded-full px-4 py-3 text-[12px] font-bold text-white disabled:opacity-40" style={{ backgroundColor: "rgb(24, 167, 208)" }}>{submitting ? "Submitting..." : quoteRequired ? "Request quote" : "Submit order"}</button>}</div></aside></div>
  </div></section>;
}
