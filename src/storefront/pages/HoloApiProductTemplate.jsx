...
import { requestAddToCart, requestResolvedConfig } from '../commerce/liveConfiguratorEngine';
...
function valueList(group = {}) {
  return asArray(group.values, group.options, group.choices, group.items, group.optionValues).map((option, index) => {
    if (typeof option === 'string' || typeof option === 'number') return { id: String(option), value: String(option), label: String(option), recommended: index === 0, default: index === 0, visible: true };
...
function apiGroups(product = {}) {
  const groups = asArray(product.optionGroups, product.metadataJson?.optionGroups, product.configurator?.optionGroups, product.configuration?.optionGroups, product.options, product.metadataJson?.options);
...
function localQuantityRows(product, qtyGroup, selections) {
  ...
  const rows = fallback.map((item, index) => {
    ...
    return { qty, quantity: qty, price, available: price > 0 || !matrixRows(product).length, recommended: Boolean(item.recommended || item.default || index === 0) };
  });
...
function optionDeliveryText(option = {}) {
  if (option.deliveryEstimate?.displayText) return option.deliveryEstimate.displayText;
...
function localDeliveryRows(product, deliveryGroup, selections, qty) {
  if (!deliveryGroup) {
  ...
  const mapped = deliveryGroup.options.filter((option) => option.visible !== false).map((option, index) => {
    ...
    return { ...option, label: option.label || option.value, description: optionDeliveryText(option), addon: option.addon || option.extra || option.priceDelta || '', price: rowPrice(pricedRow), available, selected: option.default || option.recommended || index === 0 };
  });
  const available = mapped.filter((item) => item.available !== false && item.disabled !== true);
  return available.length ? available : mapped;
}

function priceFromQuantityRow(row = {}) {
  if (row.price !== undefined && row.price !== null) return Number(row.price);
  if (row.priceMinor !== undefined && row.priceMinor !== null) return Number(row.priceMinor) / 100;
  return 0;
}

function normaliseResolvedQuantityRows(rows = []) {
  return rows.map((row, index) => ({
    ...row,
    qty: row.qty || row.quantity || row.value || row.label,
    quantity: row.quantity || row.qty || row.value || row.label,
    price: priceFromQuantityRow(row),
    recommended: Boolean(row.recommended || row.default || index === 0),
    available: row.available !== false,
  })).filter((row) => row.qty !== undefined && row.qty !== null && row.qty !== '');
}

function normaliseResolvedDeliveryRows(rows = []) {
  return rows.map((row, index) => ({
    ...row,
    value: row.value || row.id || row.label || row.day || row.name || `delivery-${index}`,
    label: row.label || row.day || row.name || row.value || `Delivery ${index + 1}`,
    description: row.description || row.latest || row.deliveryEstimate?.displayText || optionDeliveryText(row),
    available: row.available !== false && row.disabled !== true,
    selected: Boolean(row.selected || row.recommended || row.default || index === 0),
...
export default function HoloApiProductTemplate({ product = {} }) {
  const baseGroups = useMemo(() => apiGroups(product), [product]);
  const [resolvedGroups, setResolvedGroups] = useState(baseGroups);
  const groups = resolvedGroups.length ? resolvedGroups : baseGroups;
  ...
  const [selections, setSelections] = useState(() => initialSelections(baseGroups));
  const [qty, setQty] = useState('');
  const [deliveryValue, setDeliveryValue] = useState('');
  const [backendQuantityRows, setBackendQuantityRows] = useState([]);
  const [backendDeliveryRows, setBackendDeliveryRows] = useState([]);
  ...
  useEffect(() => {
    const next = initialSelections(baseGroups);
    const qRows = localQuantityRows(product, baseGroups.find(isQuantityGroup), next);
    const firstQty = qRows[0]?.qty || '1';
    const dRows = localDeliveryRows(product, baseGroups.find(isDeliveryGroup), next, firstQty);
    const firstDelivery = dRows.find((item) => item.selected)?.value || dRows[0]?.value || '';
    const nextWithDelivery = { ...next };
    const initialDeliveryGroup = baseGroups.find(isDeliveryGroup);
    if (initialDeliveryGroup && firstDelivery) nextWithDelivery[initialDeliveryGroup.key] = firstDelivery;
    setResolvedGroups(baseGroups);
    setSelections(nextWithDelivery);
    setQty(firstQty);
    setDeliveryValue(firstDelivery);
    setBackendQuantityRows([]);
    setBackendDeliveryRows([]);
    setResolverMessages([]);
  }, [product?.id, product?.slug, baseGroups]);

  const localQRows = useMemo(() => localQuantityRows(product, qtyGroup, selections), [product, qtyGroup, selections]);
  const qRows = backendQuantityRows.length ? backendQuantityRows : localQRows;
  const localDRows = useMemo(() => localDeliveryRows(product, deliveryGroup, selections, qty), [product, deliveryGroup, selections, qty]);
  const dRows = backendDeliveryRows.length ? backendDeliveryRows : localDRows;
  ...
  useEffect(() => {
    ...
    if (!qRows.some((row) => String(row.qty) === String(qty))) setQty(qRows[0].qty);
  }, [qRows, qty]);
  ...
  useEffect(() => {
    ...
    if (!stillAvailable) {
      ...
      if (deliveryGroup) setSelections((prev) => ({ ...prev, [deliveryGroup.key]: next.value }));
    }
  ...
  useEffect(() => {
    ...
    if (!qty) return;
    let cancelled = false;
    const fallbackPrice = priceFromQuantityRow(selectedRow);
    setPrice(fallbackPrice);
    setPriceStatus('checking');
    setSyncStatus('syncing');
    const timer = window.setTimeout(async () => {
      try {
        const resolved = await requestResolvedConfig({ product, selections, quantity: qty, delivery: selectedDelivery || null });
        if (cancelled) return;
        const cfg = resolved.resolvedConfig || {};
        if (Array.isArray(cfg.groups) && cfg.groups.length) setResolvedGroups(cfg.groups);
        if (resolved.selections && typeof resolved.selections === 'object') setSelections((prev) => ({ ...prev, ...resolved.selections }));
        const nextQ = normaliseResolvedQuantityRows(resolved.quantityRows || cfg.quantityRows || []);
        const nextD = normaliseResolvedDeliveryRows(resolved.deliveryRows || cfg.deliveryRows || []);
        setBackendQuantityRows(nextQ);
        setBackendDeliveryRows(nextD);
        const nextQty = resolved.raw?.quantity || cfg.selectedQuantity || nextQ[0]?.qty || qty;
        const nextDelivery = resolved.raw?.delivery || cfg.selectedDelivery || nextD.find((item) => item.selected)?.value || nextD[0]?.value || deliveryValue;
        setQty(String(nextQty));
        setDeliveryValue(String(nextDelivery || ''));
        setResolverMessages(resolved.messages || cfg.messages || []);
        setPrice(resolved.price || fallbackPrice);
        setPriceStatus('live');
        setSyncStatus('synced');
      } catch (error) {
        if (cancelled) return;
        setResolverMessages(error?.payload?.resolvedConfig?.messages || []);
        setPrice(fallbackPrice);
  ...
  }, [product, selections, qty, selectedDelivery?.value, selectedRow?.price, selectedRow?.priceMinor]);
  ...
  const chooseDelivery = (item) => {
    ...
    if (deliveryGroup) setSelections((prev) => ({ ...prev, [deliveryGroup.key]: item.value }));
  };

  const chooseOption = (group, value) => {
    setCartStatus('idle');
    setBackendQuantityRows([]);
    setBackendDeliveryRows([]);
