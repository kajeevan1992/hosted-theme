...
function normalizeCategory(item, index = 0) {
  ...
  return {
    ...
    path: `/${slug}`,
    thumbnail: item.thumbnail || item.image || '/images/hero-slide-2.svg',
    sortOrder: Number(item.sortOrder || index + 1),
    productCount: Number(item.productCount || 0),
    published: item.isActive !== false && item.published !== false,
  };
}
...
function normalizeProduct(item, categoryById = new Map(), index = 0) {
  ...
  return {
    ...
    title: name,
    name,
    description: item.description || item.subtitle || 'Browse options, upload artwork or request support before ordering.',
    image: item.thumbnail || item.image || item.imageUrl || '/images/business-card-front.svg',
  ...
  for (const base of getApiBaseCandidates()) {
    try {
      const [categoriesData, productsData] = await Promise.all([
        fetchJson(`${base}/api/internal/catalog/categories?limit=200`),
        fetchJson(`${base}/api/internal/catalog/products?limit=300`),
      ]);
      const categories = (categoriesData.items || []).map(normalizeCategory).filter((item) => item.published && item.slug);
      const byId = new Map();
      categories.forEach((category) => {
        byId.set(category.id, category);
        byId.set(category.slug, category);
      });
      const products = (productsData.items || []).map((item, index) => normalizeProduct(item, byId, index)).filter((item) => item.published && item.slug);
      return { categories, products, source: base };
    } catch (error) {
      lastError = error;
    }
  }
...
export function groupProductsByCategory(products = []) {
  return products.reduce((acc, product) => {
    const key = product.categorySlug || 'all-products';
    acc[key] = acc[key] || [];
...
export function buildNavItemsFromAdminCatalog(catalog, fallbackItems = []) {
  ...
  const items = categories.slice(0, 9).map((category) => {
    ...
    const links = products.length
      ? products.map((product) => [product.title, product.path])
      : [[`Browse ${category.name}`, category.path], ['Request quote', '/bespoke-quote']];
    return {
      label: category.name,
      path: category.path,
      feature: {
        title: category.name,
        body: category.description || `Browse ${category.name} products, upload artwork or request quote support.`,
        image: category.thumbnail || products[0]?.image || '/images/hero-slide-2.svg',
        cta: `View ${category.name}`,
      },
      columns: [
        { title: 'Products', links: links.slice(0, 6) },
        { title: 'More options', links: links.slice(6, 12).length ? links.slice(6, 12) : [['All products', '/all-products'], ['Bespoke quote', '/bespoke-quote']] },
        { title: 'Support', links: [['Artwork help', '/artwork-upload'], ['Custom quote', '/bespoke-quote'], ['Cart', '/cart'], ['Contact', '/bespoke-quote']] },
      ],
    };
  });
  ...
  items.push({
    label: 'All Products',
    path: '/all-products',
    feature: { title: 'Explore the full catalog', body: 'Browse all admin-connected products and categories.', image: '/images/hero-slide-2.svg', cta: 'Shop all products' },
    columns: [
      { title: 'Categories', links: categories.slice(0, 8).map((category) => [category.name, category.path]) },
      { title: 'Popular products', links: (catalog.products || []).slice(0, 8).map((product) => [product.title, product.path]) },
      { title: 'Support', links: [['Bespoke quote', '/bespoke-quote'], ['Artwork upload', '/artwork-upload'], ['Cart', '/cart']] },
    ],
  });
