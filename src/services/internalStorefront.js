...
function errorMessage(payload, path) { const error = payload?.error; if (typeof error === 'string') return error; if (error?.message) return error.message; return `Internal storefront request failed: ${path}`; }
async function request(path, { method = 'GET', body, params, customer = false } = {}) {
  ...
  if (!response.ok || payload?.ok === false) throw new Error(errorMessage(payload, path));
  return payload;
}
...
async function uploadMultipart(path, formData, params = {}) { const response = await fetch(buildUrl(path, params), { method: 'POST', body: formData, credentials: 'include' }); const payload = await response.json().catch(() => null); if (!response.ok || payload?.ok === false) throw new Error(errorMessage(payload, path)); return payload; }
