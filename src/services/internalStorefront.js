...
async function request(path, { method = 'GET', body, params, customer = false } = {}) {
  ...
  const response = await fetch(buildUrl(path, params), {
    method,
    headers: body || customer ? { ...(body ? { 'Content-Type': 'application/json' } : {}), ...(customer && email ? { 'X-Customer-Email': email } : {}) } : undefined,
...
export async function createInternalOrder(payload) {
  ...
  if (customerEmailFromPayload) setCustomerEmail(customerEmailFromPayload);
  const body = { ...payload, artworkUploadIds: [...new Set([...(payload?.artworkUploadIds || []), artworkUploadId].filter(Boolean))] };
  const response = await request('/api/internal/orders', { method: 'POST', body });
