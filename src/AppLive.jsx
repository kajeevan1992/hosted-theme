...
import {
  ...
  updateCart as serviceUpdateCart,
...
function toThemeCartItems(payload) {
  const data = payload || {};
  const items = Array.isArray(data.items) ? data.items : [];
  return items.map((item) => ({
    id: item.id || item.cartItemId || crypto.randomUUID(),
    name: item.name || item.productName || item.productSlug || "Print product",
    productId: item.productId,
    productSlug: item.productSlug,
    config: item.config || item.selections || {},
    selections: item.selections || item.config || {},
    qty: Number(item.qty || item.quantity || 1),
    quantity: Number(item.quantity || item.qty || 1),
    price: Number(item.price || item.pricePreviewMinor / 100 || item.pricing?.grossTotalMinor / 100 || item.pricing?.netTotalMinor / 100 || 0),
...
function useBackendProducts() {
  ...
  useEffect(() => {
    ...
    listProducts()
      .then((result) => {
        if (!cancelled) setBackendProducts(Array.isArray(result?.items) ? result.items : []);
      })
      .catch(() => {
...
function useCart() {
  ...
  const refresh = async () => {
    try {
      ...
      const mapped = toThemeCartItems(data);
      setItems(mapped);
      setServerTotals(data?.totals || null);
      localStorage.setItem("holo-cart", JSON.stringify(mapped));
      return data;
    } catch {
      return null;
    }
  ...
  const addItem = async (item) => {
    const localItem = { ...item, id: crypto.randomUUID(), qty: item.qty || item.quantity || 1 };
    setItems((prev) => [...prev, localItem]);
    try {
      const data = await serviceAddToCart({
        productId: item.productId,
        productSlug: item.productSlug,
        productName: item.name,
        quantity: item.quantity || item.qty || item.config?.quantity || 1,
        selections: item.selections || item.config || {},
        delivery: item.delivery,
        pricePreviewMinor: Math.round(Number(item.price || 0) * 100),
      });
      const mapped = toThemeCartItems(data);
  ...
  const removeItem = async (id) => {
    ...
    try { await serviceRemoveFromCart(id); await refresh(); } catch {}
  };
  ...
  const updateQty = async (id, delta) => {
    ...
    setItems((prev) => prev.map((x) => {
      ...
      return { ...x, qty: nextQty, quantity: nextQty };
    }));
    try { await serviceUpdateCart({ id, cartItemId: id, quantity: nextQty }); await refresh(); } catch {}
  };

  const subtotal = serverTotals?.grossTotalMinor != null
    ? Number(serverTotals.grossTotalMinor) / 100
    : items.reduce((sum, item) => sum + Number(item.price || 0) * Number(item.qty || 1), 0);
  return { items, addItem, removeItem, updateQty, subtotal, refresh, totals: serverTotals };
}
...
function firstImageFor(product, fallbackImages = []) {
  const media = product?.media || product?.metadataJson?.media || {};
  const images = [media.heroImageUrl, ...(Array.isArray(media.gallery) ? media.gallery : [])].filter(Boolean);
  return images.length ? images : fallbackImages;
}

function findBackendProduct(type, backendProducts) {
  const wanted = TYPE_TO_SLUG[type];
  return backendProducts.find((product) => {
    const slug = String(product.slug || "").replace(/^\//, "");
    return slug === wanted || slug.includes(wanted) || String(product.productType || "") === type;
  });
}

function optionValues(option) {
  const values = option.values || option.options || option.choices || [];
  if (!Array.isArray(values) || !values.length) return [{ value: "standard", label: "Standard" }];
  return values.map((value) => typeof value === "string" || typeof value === "number" ? { value, label: String(value) } : value);
}

function mapBackendPage(backendProduct, fallbackPage) {
  if (!backendProduct) return fallbackPage;
  const content = backendProduct.content || backendProduct.metadataJson?.content || {};
  const options = Array.isArray(backendProduct.options) ? backendProduct.options : [];
  const quantities = Array.isArray(backendProduct.quantities) ? backendProduct.quantities : [];
  const deliveryServices = backendProduct.delivery?.services || backendProduct.metadataJson?.delivery?.services || [];
  const optionGroups = options.length ? options.map((option) => ({
    key: option.id,
    label: option.label || option.name || option.id,
    valueLabel: option.valueLabel || optionValues(option)[0]?.label || optionValues(option)[0]?.value || "",
    style: ["image-card-grid", "text-card-grid", "swatch-card-grid", "tile"].includes(option.display || option.displayType || option.type) ? "tile" : "pill",
    options: optionValues(option).map((value) => ({
      value: value.value,
      sublabel: value.subtitle || value.sublabel || value.helpText || "",
      recommended: Boolean(value.recommended || value.default || value.badge === "Recommended"),
      muted: Boolean(value.disabled),
    })),
  })) : fallbackPage.optionGroups;
  const mappedQuantities = quantities.length ? quantities.map((quantity, index) => {
    const qty = typeof quantity === "number" ? quantity : Number(quantity.qty || quantity.quantity || quantity.value || 1);
    const priceMinor = typeof quantity === "object" ? Number(quantity.priceMinor || quantity.priceFromMinor || 0) : 0;
    return {
      qty,
      price: priceMinor ? priceMinor / 100 : Number(backendProduct.priceFromMinor || 0) / 100,
      recommended: typeof quantity === "object" ? Boolean(quantity.recommended || quantity.default) : index === 0,
    };
  }) : fallbackPage.quantities;
  const mappedDelivery = Array.isArray(deliveryServices) && deliveryServices.length ? deliveryServices.filter((item) => item.enabled !== false).map((item, index) => ({
    day: item.label || item.id || "Delivery",
    latest: item.workingDays === 0 ? `Cutoff ${item.cutoff || "today"}` : `Expected in ${item.workingDays || 0} working day(s)`,
    addon: item.extraMinor ? `+ ${currency(Number(item.extraMinor) / 100)}` : null,
    selected: index === 0,
  })) : fallbackPage.deliveryOptions;
  return {
    ...fallbackPage,
    tabs: fallbackPage.tabs,
    optionGroups,
    quantities: mappedQuantities,
    deliveryOptions: mappedDelivery,
    description: content.longDescription || content.shortDescription || backendProduct.description || fallbackPage.description,
    bullets: Array.isArray(content.designGuidelines) && content.designGuidelines.length ? content.designGuidelines : fallbackPage.bullets,
    specs: Array.isArray(content.specifications) && content.specifications.length ? content.specifications.map((item) => [item.label, item.value]) : fallbackPage.specs,
    guidelines: Array.isArray(content.designGuidelines) && content.designGuidelines.length ? content.designGuidelines : fallbackPage.guidelines,
    faqs: Array.isArray(content.faqs) && content.faqs.length ? content.faqs.map((item) => item.question || item.label || item) : fallbackPage.faqs,
  };
}

function mapBackendProduct(backendProduct, fallbackProduct) {
  if (!backendProduct) return fallbackProduct;
  const images = firstImageFor(backendProduct, fallbackProduct.images);
  return {
    ...fallbackProduct,
    name: backendProduct.name || fallbackProduct.name,
    slug: backendProduct.slug ? `/${backendProduct.slug}` : fallbackProduct.slug,
    basePrice: Number(backendProduct.priceFromMinor || 0) / 100 || fallbackProduct.basePrice,
    images,
    description: backendProduct.content?.shortDescription || backendProduct.description || fallbackProduct.description,
  };
}
function ProductPage({ type, cart, backendProducts = [] }) {
  const backendProduct = findBackendProduct(type, backendProducts);
  const product = mapBackendProduct(backendProduct, catalog[type]);
  const page = mapBackendPage(backendProduct, productPageContent[type]);
  ...
  const [ruleState, setRuleState] = useState({ blocked: false, messages: [], priceAdjustments: [] });
  ...
  const handleOptionSelect = async (key, value) => {
    ...
    try {
      const result = await evaluateProductRules({ productId: backendProduct.id, selections: next });
      if (result?.selections) setSelected((prev) => ({ ...prev, ...result.selections }));
      setRuleState({ blocked: Boolean(result?.blocked), messages: result?.messages || [], priceAdjustments: result?.priceAdjustments || [] });
    } catch {
      setRuleState((prev) => ({ ...prev, messages: [] }));
    }
  };

  const currentPrice = (page.quantities.find((q) => q.qty === selectedQty)?.price ?? product.basePrice) + (ruleState.priceAdjustments || []).reduce((sum, item) => sum + Number(item.amountMinor || 0) / 100, 0);
      ...
      <Shell narrow>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.02fr_1fr]">
          ...
          <div className="space-y-5">
            ...
            {ruleState.messages.length > 0 && (
              <div className="rounded-[14px] border bg-white p-4 text-[12px] shadow-[0_8px_18px_rgba(0,0,0,0.03)]" style={{ borderColor: ruleState.blocked ? "#F3B4B4" : BRAND.line, color: ruleState.blocked ? "#9F1D1D" : BRAND.muted }}>
                {ruleState.messages.map((message, index) => <div key={`${message.text}-${index}`}>{message.text}</div>)}
              </div>
            )}
            ...
            <div className="rounded-[20px] border bg-white p-5 shadow-[0_16px_34px_rgba(0,0,0,0.04)]" style={{ borderColor: BRAND.line }}>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
                <PrimaryButton className="justify-center" onClick={() => !ruleState.blocked && cart.addItem({ name: product.name, productId: backendProduct?.id, productSlug: backendProduct?.slug || product.slug?.replace(/^\//, ""), config: { ...selected, quantity: selectedQty }, selections: selected, quantity: selectedQty, delivery: page.deliveryOptions[selectedDelivery], price: currentPrice })}>
                  {ruleState.blocked ? "Fix options first" : "Add to cart"}
                </PrimaryButton>
...
export default function App() {
  ...
  switch (path) {
    case "/standard-business-cards":
      page = <ProductPage type="businessCards" cart={cart} backendProducts={backendProducts} />;
      break;
    case "/flyers":
      page = <ProductPage type="flyers" cart={cart} backendProducts={backendProducts} />;
      break;
    case "/posters-large-format-prints":
      page = <ProductPage type="posters" cart={cart} backendProducts={backendProducts} />;
      break;
    case "/booklets":
