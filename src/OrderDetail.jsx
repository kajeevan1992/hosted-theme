...
function minorToMoney(value){return Number(value||0)/100;}
function taxSummary(order={}){const t=order.taxSummary||{};return {net:t.net??minorToMoney(order.subtotalMinor),vat:t.vat??minorToMoney(order.taxMinor),delivery:t.delivery??minorToMoney(order.shippingMinor),gross:t.gross??(order.total||minorToMoney(order.totalMinor)),breakdown:Array.isArray(t.vatBreakdown)?t.vatBreakdown:Array.isArray(order.vatBreakdown)?order.vatBreakdown:[]};}
...
function VatSummary({order}){const t=taxSummary(order);return <Card title="VAT summary"><div className="grid gap-2"><div className="flex justify-between"><span>Net subtotal</span><b className="text-[#161A22]">{currency(t.net)}</b></div><div className="flex justify-between"><span>VAT</span><b className="text-[#161A22]">{currency(t.vat)}</b></div><div className="flex justify-between"><span>Delivery gross</span><b className="text-[#161A22]">{currency(t.delivery)}</b></div><div className="flex justify-between border-t pt-2" style={{borderColor:'#E3E8F0'}}><span>Total inc. VAT</span><b className="text-[#161A22]">{currency(t.gross)}</b></div>{t.breakdown?.length?<div className="mt-2 rounded-[12px] border bg-[#FBFCFF] px-3 py-2" style={{borderColor:'#E3E8F0'}}><div className="font-bold text-[#161A22]">VAT breakdown</div>{t.breakdown.map((row)=><div key={row.rate} className="flex justify-between"><span>{row.rate}% VAT</span><span>{currency(row.vat??minorToMoney(row.vatMinor))}</span></div>)}</div>:<div className="mt-2">VAT breakdown will appear once the order is fully synced.</div>}</div></Card>}
  ...
  return <section className="py-6">
    <div className="mx-auto max-w-[1100px] px-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card title="Items">{(order.items||[]).length?order.items.map((item)=><div key={item.id||item.productName} className="mb-2 flex justify-between gap-3"><span>{item.productName||item.titleSnapshot||'Print item'} × {item.quantity||1}<span className="ml-2 text-[10px] uppercase tracking-[0.08em] text-[#18A7D0]">{item.vatRate!==undefined?`${item.vatRate}% VAT`:''}</span></span><span>{currency(item.totalPrice||minorToMoney(item.grossTotalMinor))}</span></div>):<div>Order item details are syncing.</div>}<div className="mt-3 font-bold text-[#161A22]">Total {currency(tax.gross)}</div></Card>
