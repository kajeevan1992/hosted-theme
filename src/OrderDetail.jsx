...
function tone(status){const s=String(status||'').toLowerCase();if(s.includes('approved')||s.includes('production')||s.includes('dispatch'))return '#059669';if(s.includes('pending')||s.includes('await')||s.includes('quality'))return '#B45309';if(s.includes('reject')||s.includes('replacement')||s.includes('block'))return '#DC2626';return '#18A7D0';}
  ...
  return <section className="py-6">
    <div className="mx-auto max-w-[1100px] px-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card title="Documents"><button className="text-[11px] border px-3 py-1 rounded-full">Invoice</button><div className="mt-2">Invoices/proofs can be connected here later using the same order ID.</div></Card>
