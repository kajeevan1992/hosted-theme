import { attachOrderResponseArtwork, createInternalOrder, uploadArtworkFile, listCustomerOrders, getCustomerOrder, setCustomerEmail, getCustomerEmail } from './services/internalStorefront';
...
export const checkout = { async precheck() { return call('checkout.precheck', storefront().checkout?.precheck, null); }, async createDraft(data = {}) { return call('checkout.createDraft', () => storefront().checkout?.createDraft?.(data), null); }, async createOrder(data = {}) { const hostedCreate = storefront().checkout?.createOrder; if (typeof hostedCreate === 'function') { const response = await hostedCreate(data); return attachOrderResponseArtwork(response, data); } return createInternalOrder(data); }, async finalise(data = {}) { const hostedFinalise = storefront().checkout?.finalise; if (typeof hostedFinalise === 'function') { const response = await hostedFinalise(data); return attachOrderResponseArtwork(response, data); } return createInternalOrder(data); } };
export const artwork = { async upload(dataOrFile, meta = {}) { const payload = dataOrFile instanceof File ? { file: dataOrFile, ...meta } : { ...(dataOrFile || {}), ...meta }; const hostedUpload = storefront().artwork?.upload; if (typeof hostedUpload === 'function') return hostedUpload(payload); if (payload.file instanceof File) return uploadArtworkFile(payload.file, payload); return null; }, async preflight(data = {}) { return call('artwork.preflight', () => storefront().artwork?.preflight?.(data), null); } };
